...
export default function ResetPasswordContent({ dict, locale }: { dict: Record<string, string>, locale: string }) {
  ...
  useEffect(() => {
    if (!code) {
      setErrorMessage(dict['auth.invalid_or_missing_reset_code'] || "Invalid or missing reset code. Please request a new password reset link.");
    }
  }, [code, dict]);

  const handleResetPassword = async (prevState: any, formData: FormData) => {
    if (!code) {
      return { message: dict['auth.invalid_reset_code'] || "Invalid reset code" };
    }
              ...
              <div className="relative z-10 pt-24 pb-8 max-w-xl mx-auto h-full w-full flex flex-col gap-2 items-center justify-center">
                <div className="flex flex-col items-center text-center">
                  ...
                  <h1 className="text-3xl md:text-4xl lg:text-5xl font-medium tracking-tighter text-center text-balance text-primary mb-4">
                    {dict['auth.password_reset_complete'] || "Password Reset Complete"}
                  </h1>
                  <p className="text-base md:text-lg text-center text-muted-foreground font-medium text-balance leading-relaxed tracking-tight max-w-md mb-6">
                    {dict['auth.password_reset_success'] || "Your password has been successfully updated. You can now sign in with your new password."}
                  </p>
                  <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
                    ...
                    >
                      {dict['auth.go_to_sign_in'] || "Go to sign in"}
                    </Link>
                  </div>
                </div>
              </div>
          ...
          <div className="relative flex flex-col items-center w-full px-6">
            ...
            <div className="relative z-10 pt-24 pb-8 max-w-md mx-auto h-full w-full flex flex-col gap-2 items-center justify-center">
              ...
              >
                ...
                <span className="font-medium text-muted-foreground text-xs tracking-wide">{dict['auth.back_to_sign_in'] || "Back to sign in"}</span>
              </Link>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-medium tracking-tighter text-center text-balance text-primary">
                {dict['auth.reset_password'] || "Reset Password"}
              </h1>
              <p className="text-base md:text-lg text-center text-muted-foreground font-medium text-balance leading-relaxed tracking-tight mt-2 mb-6">
                {dict['auth.create_new_password'] || "Create a new password for your account"}
              </p>
            </div>
          </div>
          {/* Form card */}
            ...
            <div className="w-full max-w-md rounded-xl bg-[#F3F4F6] dark:bg-[#F9FAFB]/[0.02] border border-border p-8">
                ...
                <form className="space-y-4">
                  <div>
                    <Input
                      ...
                      name="password"
                      type="password"
                      placeholder={dict['auth.new_password'] || "New password"}
                  ...
                  <div>
                    <Input
                      ...
                      type="password"
                      placeholder={dict['auth.confirm_new_password'] || "Confirm new password"}
                      className="h-12 rounded-full bg-background border-border"
                      required
                    />
                  </div>
                  <div className="space-y-4 pt-4">
                    <SubmitButton
                      ...
                      pendingText={dict['auth.updating_password'] || "Updating password..."}
                    >
                      {dict['auth.reset_password'] || "Reset Password"}
                    </SubmitButton>
                  </div>
              ...
              {errorMessage && (
                <div className="mt-6 flex justify-center">
                  ...
                  >
                    {dict['auth.return_to_sign_in'] || "Return to sign in"}
                  </Link>
                </div>
              )}
            </div>
