...
export default function LoginContent({ dict, locale }: { dict: Record<string, string>, locale: string }) {
  ...
  useEffect(() => {
    if (!isLoading && user) {
      router.push(returnUrl || `/${locale}/dashboard`);
    }
  }, [user, isLoading, router, returnUrl, locale]);
  ...
  const handleSignIn = async (prevState: any, formData: FormData) => {
    if (returnUrl) {
      ...
      formData.append("returnUrl", `/${locale}/dashboard`);
    }
    const result = await signIn(prevState, formData, locale);
  ...
  const handleForgotPassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!forgotPasswordEmail || !forgotPasswordEmail.includes('@')) {
      setForgotPasswordStatus({
        success: false,
        message: t('error.invalid_email') || "Please enter a valid email address"
      });
      return;
    }
    ...
    formData.append("origin", window.location.origin);
    const result = await forgotPassword(null, formData, locale, dict);
  ...
  // TEMP: Commented out defensive check for debugging
  // if (!dict || !dict['auth.sign_in_to_continue']) {
  //   return (
              ...
              <div className="relative z-10 pt-24 pb-8 max-w-xl mx-auto h-full w-full flex flex-col gap-2 items-center justify-center">
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-base md:text-lg text-center text-muted-foreground font-medium text-balance leading-relaxed tracking-tight max-w-md mb-2">
                    {t('auth.confirmation_link_sent') || "We've sent a confirmation link to:"}
                  </p>
                  <p className="text-lg font-medium mb-6">
                    {registrationEmail || t('auth.your_email_address') || "your email address"}
                  </p>
                  <div className="bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/50 rounded-lg p-6 mb-8 max-w-md w-full">
                    <p className="text-sm text-green-800 dark:text-green-400 leading-relaxed">
                      {t('auth.check_email_instructions') || "Click the link in the email to activate your account. If you don't see the email, check your spam folder."}
                    </p>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
                    ...
                    >
                      {t('auth.return_to_home') || "Return to home"}
                    </Link>
                    <button
          ...
          <div className="relative flex flex-col items-center w-full px-6">
            ...
            <div className="relative z-10 pt-24 pb-8 max-w-md mx-auto h-full w-full flex flex-col gap-2 items-center justify-center">
              ...
              >
                ...
                <span className="font-medium text-muted-foreground text-xs tracking-wide">{t('auth.back_to_home')}</span>
              </Link>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-medium tracking-tighter text-center text-balance text-primary">
                {isSignUp ? t('auth.join_suna') : t('auth.welcome_back')}
              </h1>
              {/*
              <p className="text-base md:text-lg text-center text-muted-foreground font-medium text-balance leading-relaxed tracking-tight mt-2 mb-6">
                {isSignUp ? t('auth.create_account_and_start') : t('auth.sign_in_to_continue')}
              </p>
              */}
            </div>
          </div>
          <div className="relative z-10 flex justify-center px-6 pb-24">
            <div className="w-full max-w-md rounded-xl bg-[#F3F4F6] dark:bg-[#F9FAFB]/[0.02] border border-border p-8">
                ...
                <div className="space-y-4 pt-4">
                  {!isSignUp ? (
                    <>
                      ...
                      <Link
                        href={`/${locale}/auth?mode=signup${returnUrl ? `&returnUrl=${returnUrl}` : ''}`}
                        className="flex h-12 items-center justify-center w-full text-center rounded-full border border-border bg-background hover:bg-accent/20 transition-all"
                      >
                    ...
                    <>
                      <SubmitButton
                        ...
                        className="w-full h-12 rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-all shadow-md"
                        pendingText={t('auth.creating_account')}
                      >
                        {t('auth.sign_up')}
                      </SubmitButton>
                      <Link
                        href={`/${locale}/auth${returnUrl ? `?returnUrl=${returnUrl}` : ''}`}
                        className="flex h-12 items-center justify-center w-full text-center rounded-full border border-border bg-background hover:bg-accent/20 transition-all"
                      >
                        {t('auth.back_to_sign_in')}
                      </Link>
              ...
              <div className="mt-8 text-center text-xs text-muted-foreground">
                ...
                {t('and')}{' '}<Link href={`/${locale}/privacy`} className="text-primary hover:underline">{t('privacy_policy')}</Link>
              </div>
            </div>
          </div>
        </section>
        ...
        <DialogContent className="sm:max-w-md rounded-xl bg-[#F3F4F6] dark:bg-[#F9FAFB]/[0.02] border border-border">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle className="text-xl font-medium">{t('auth.reset_password')}</DialogTitle>
              <button
