        ...
        <div className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-base font-medium text-zinc-900 dark:text-zinc-100">
                {t('portExposure.portExposure', 'Port Exposure')}
              </CardTitle>
            </div>
          </div>
          ...
          {!isStreaming && (
            ...
            >
              ...
              {actualIsSuccess ? t('portExposure.portExposedSuccessfully', 'Port exposed successfully') : t('portExposure.failedToExposePort', 'Failed to expose port')}
            </Badge>
          )}
        </div>
      </CardHeader>
                  ...
                  <div className="p-4">
                    <div className="flex items-start gap-3 mb-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-zinc-800 dark:text-zinc-200 mb-2">
                          {t('portExposure.exposedUrl', 'Exposed URL')}
                        </h3>
                        <a
                          href={url}
                          target="_blank"
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-xs bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/50 rounded-md p-3 text-amber-600 dark:text-amber-400 flex items-start gap-2">
                        ...
                        <span>{t('portExposure.urlTemporaryWarning', 'This URL might only be temporarily available and could expire after some time.')}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {!port && !url && !isStreaming && (
                <div className="flex flex-col items-center justify-center py-12 px-6">
                  ...
                  <h3 className="text-xl font-semibold mb-2 text-zinc-900 dark:text-zinc-100">
                    {t('portExposure.noPortInformation', 'No Port Information')}
                  </h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center max-w-md">
                    {t('portExposure.noPortInfoDescription', 'No port exposure information is available yet. Use the expose-port command to share a local port.')}
                  </p>
                </div>
              )}
            </div>
