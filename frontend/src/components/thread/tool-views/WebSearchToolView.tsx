          ...
          <div className="flex items-center p-2 bg-zinc-100 dark:bg-zinc-900 justify-between border-b border-zinc-200 dark:border-zinc-800">
            ...
            {!isStreaming && (
              <span className={cn(
                ...
                {isSuccess ? t('webSearch.success', 'Success') : t('webSearch.failed', 'Failed')}
              </span>
            )}
          </div>
          
          <div className="px-3 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900">
            <code className="text-xs font-mono text-zinc-700 dark:text-zinc-300">{query || t('webSearch.unknownQuery', 'Unknown query')}</code>
          </div>
            ...
            {isStreaming ? (
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center p-6">
                  ...
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('webSearch.searching', 'Searching the web...')}</p>
                  <p className="text-xs mt-1 text-zinc-500 dark:text-zinc-400">{t('webSearch.mightTakeAMoment', 'This might take a moment')}</p>
                </div>
              </div>
            ) : searchResults.length > 0 ? (
              <div className="p-3">
                <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-3">
                  {t('webSearch.foundResults', 'Found {{count}} results', { count: searchResults.length })}
                </div>
              ...
              <div className="p-6 text-center flex-1 flex flex-col items-center justify-center h-full">
                ...
                <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('webSearch.noResults', 'No results found')}</p>
                <p className="text-xs mt-1 text-zinc-500 dark:text-zinc-400">{t('webSearch.tryRefining', 'Try refining your search query')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
          {!isStreaming && (
            <div className="flex items-center gap-2">
              ...
              <span>
                {isSuccess 
                  ? t('webSearch.completed', '{{tool}} completed successfully', { tool: toolTitle }) 
                  : t('webSearch.failedOperation', '{{tool}} operation failed', { tool: toolTitle })}
              </span>
            </div>
          )}
          ...
          {isStreaming && (
            <div className="flex items-center gap-2">
              ...
              <span>{t('webSearch.executing', 'Executing')} {toolTitle.toLowerCase()}...</span>
            </div>
          )}
