...
export function WebScrapeToolView({ 
  ...
  const domain = url ? formatDomain(url) : t('webScrape.unknown', 'Unknown');
            ...
            {!isStreaming && (
              <div className="flex items-center gap-2">
                <span className={cn(
                  ...
                  {isSuccess ? t('webScrape.success', 'Success') : t('webScrape.failed', 'Failed')}
                </span>
                ...
                >
                  ...
                  <span>{t('webScrape.openUrl', 'Open URL')}</span>
                </a>
              </div>
            )}
          </div>
          ...
          {isStreaming ? (
            <div className="flex-1 bg-white dark:bg-zinc-950 flex items-center justify-center">
              <div className="text-center p-6">
                ...
                <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('webScrape.scraping', 'Scraping webpage...')}</p>
                <p className="text-xs mt-1 text-zinc-500 dark:text-zinc-400">{t('webScrape.fetching', 'Fetching content from {{domain}}', { domain })}</p>
              </div>
            </div>
          ) : (
            <div className="flex-1 overflow-auto bg-white dark:bg-zinc-950 font-mono text-sm">
              {webpageContent ? (
                <div className="p-3">
                  <div className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2">{t('webScrape.pageContent', 'Page Content')}</div>
                  <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md">
                    <pre className="p-3 text-xs overflow-auto whitespace-pre-wrap text-zinc-800 dark:text-zinc-300 font-mono">
                      {webpageContent.text || t('webScrape.noContent', 'No content extracted')}
                    </pre>
                  </div>                  
                </div>
              ) : (
                <div className="p-6 h-full flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{t('webScrape.noContent', 'No content extracted')}</p>
                    <p className="text-xs mt-1 text-zinc-500 dark:text-zinc-400">{t('webScrape.restricted', 'The webpage might be restricted or empty')}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
          {!isStreaming && (
            <div className="flex items-center gap-2">
              ...
              <span>
                {isSuccess 
                  ? t('webScrape.completed', '{{tool}} completed successfully', { tool: toolTitle }) 
                  : t('webScrape.failedOperation', '{{tool}} operation failed', { tool: toolTitle })}
              </span>
            </div>
          )}
          ...
          {isStreaming && (
            <div className="flex items-center gap-2">
              ...
              <span>{t('webScrape.executing', 'Executing')} {toolTitle.toLowerCase()}...</span>
            </div>
          )}
