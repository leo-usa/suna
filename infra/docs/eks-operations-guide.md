...
```
User request
  → Cloudflare DNS (api-eks.dobby.now)
...
The EKS cluster is the main production target. Lightsail and ECS are still deployed to, but EKS handles the real traffic through `api-eks.dobby.now`.
...
- **Uptime:** Set up HTTP monitors for `api-eks.dobby.now`
