...
interface AvatarProps extends ViewProps {
  ...
  /** Use Dobby symbol instead of icon (for SUNA agent) */
  useDobbySymbol?: boolean;
 ...
 * - Dobby symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Dobby symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Dobby symbol always uses solid black bg with white icon
    if (useDobbySymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useDobbySymbol ? 0 : (showBorder ? 1.5 : 0),
