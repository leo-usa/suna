...
export function DobbyComputer({
  ...
  log.log('[DobbyComputer] Render - toolMessages:', toolMessages.length, 'currentIndex:', currentIndex);
  log.log('[DobbyComputer] project:', project ? { id: project.id, name: project.name, hasSandbox: !!project.sandbox } : 'undefined');
  log.log('[DobbyComputer] sandboxId prop:', sandboxId);
  ...
  log.log('[DobbyComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[DobbyComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[DobbyComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[DobbyComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[DobbyComputer] Calling extractToolCallAndResult');
