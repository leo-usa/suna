...
export function useChat(): UseChatReturn {
  ...
  // Prefer mode wrappers (Basic / Advanced); backend remaps free Basic at run time
  ...
  useEffect(() => {
    ...
    // CRITICAL: Only allow dobby/basic or dobby/power models
    // Other models (like dobby/kimi-k2.5) are internal and should not be used
  ...
  const loadThread = useCallback((threadId: string) => {
    ...
    // Reset Dobby Computer state when switching threads
    useDobbyComputerStore.getState().reset();
    log.log('[useChat] Reset Dobby Computer state');
  ...
  const startNewChat = useCallback(() => {
    ...
    // Reset Dobby Computer state when starting new chat
    useDobbyComputerStore.getState().reset();
    log.log('[useChat] Reset Dobby Computer state for new chat');
