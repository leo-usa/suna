...
/**
 * Check if a model ID corresponds to a Dobby mode (Basic or Advanced)
 */
export function isKortixMode(modelId: string): boolean {
  // New Dobby registry IDs
  if (modelId === 'dobby/basic' || modelId === 'dobby/power' ||
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    kortix: KortixSymbolIcon, // Dobby modes use the Dobby symbol
