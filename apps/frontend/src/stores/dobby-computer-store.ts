...
interface DobbyComputerState {
  ...
  // Clear pending tool nav after DobbyComputer processes it
...
export const useDobbyComputerStore = create<DobbyComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[DobbyComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
          set({
      ...
      clearFileState: () => {
        console.log('[DobbyComputerStore] Clearing file state');
        set({
          ...initialFileState,
        });
      },
      ...
      reset: () => {
        console.log('[DobbyComputerStore] Full reset');
        set(initialState);
      },
    }),
    {
      name: 'dobby-computer-store',
    }
  )
);
...
// Sandbox context
export const useDobbyComputerSandboxId = () =>
  useDobbyComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useDobbyComputerStore((state) => state.setSandboxContext);

// Main view state
export const useDobbyComputerActiveView = () => 
  useDobbyComputerStore((state) => state.activeView);

// Individual selectors for files state (stable, primitive values)
export const useDobbyComputerFilesSubView = () =>
  useDobbyComputerStore((state) => state.filesSubView);

export const useDobbyComputerCurrentPath = () =>
  useDobbyComputerStore((state) => state.currentPath);

export const useDobbyComputerSelectedFilePath = () =>
  useDobbyComputerStore((state) => state.selectedFilePath);

export const useDobbyComputerFilePathList = () =>
  useDobbyComputerStore((state) => state.filePathList);

export const useDobbyComputerCurrentFileIndex = () =>
  useDobbyComputerStore((state) => state.currentFileIndex);

// Legacy combined selector (for backward compatibility) - use individual selectors in components
export const useDobbyComputerFilesState = () => ({
  filesSubView: useDobbyComputerStore((state) => state.filesSubView),
  currentPath: useDobbyComputerStore((state) => state.currentPath),
  selectedFilePath: useDobbyComputerStore((state) => state.selectedFilePath),
  filePathList: useDobbyComputerStore((state) => state.filePathList),
  currentFileIndex: useDobbyComputerStore((state) => state.currentFileIndex),
});

// Actions are stable references (functions don't change)
export const useDobbyComputerActions = () =>
  useDobbyComputerStore((state) => ({
...
export const useDobbyComputerPendingToolNavIndex = () =>
  useDobbyComputerStore((state) => state.pendingToolNavIndex);

export const useDobbyComputerClearPendingToolNav = () =>
  useDobbyComputerStore((state) => state.clearPendingToolNav);

// Side panel state selectors
export const useIsSidePanelOpen = () =>
  useDobbyComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useDobbyComputerStore((state) => state.setIsSidePanelOpen);
