...
export default function FeishuBotDashboardPage() {
  const t = useTranslations('feishuBotPage');
  ...
  const linkQuery = useQuery({
    queryKey: ['feishu-bot-link-status', accountId],
    queryFn: async () => {
      const res = await backendApi.get<LinkStatus>(
        `/integrations/feishu-bot/link-status?account_id=${encodeURIComponent(accountId)}`,
        { showErrors: false },
      );
  ...
  const pairingMutation = useMutation({
    mutationFn: async () => {
      const res = await backendApi.post<PairingResponse>(
        '/integrations/feishu-bot/pairing-code',
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['feishu-bot-link-status', accountId] });
    },
  });
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {linkQuery.data && (
              <p className="text-sm text-muted-foreground">
                {t('linkedSessions')}:{' '}
                <span className="font-medium text-foreground">{linkQuery.data.linked_peer_count}</span>
              </p>
            )}
