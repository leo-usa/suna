...
import { 
  ...
  useDobbyComputerPendingToolNavIndex,
  useDobbyComputerClearPendingToolNav,
} from '@/stores/dobby-computer-store';
...
const FLOATING_LAYOUT_ID = 'dobby-computer-float';

export const DobbyComputer = memo(function DobbyComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[DobbyComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the Dobby Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the Dobby Computer.
