...
2. Use the image_edit_or_generate tool with the expanded prompt and aspect_ratio="landscape" for 16:9 aspect ratio (unless otherwise specified)
3. After the image generation tool returns the generated image, update the HTML file by replacing each dummy image link with the local sandbox path to the generated image
4. **Save and link generated images directly in the current workspace directory (not in a subdirectory)**
