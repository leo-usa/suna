...
- **Image Generation Workflow for Reports:**
  ...
  3. Use the Replicate image generation tool (`<replicate-generate-image>`) with the expanded prompt and a 16:9 aspect ratio (unless otherwise specified).
  4. After the Replicate tool returns the generated image, update the HTML file by replacing each dummy image link with the local sandbox path to the generated image. **Save and link Replicate-generated images directly in the current workspace directory (not in a 'replicate' subdirectory).**
  5. The final HTML report must have all images generated by Replicate, properly linked and ready for download/viewing.
...
- **PPT Generation Workflow:**
  ...
  4. For each picture placeholder, expand the description into a detailed, context-specific English prompt and use the Replicate image generation tool to generate the image.
