...
from agent.tools.sb_tts_tool import SandboxTTSTool
...
async def run_agent(
    ...
    if enabled_tools is None:
        ...
        thread_manager.add_tool(SandboxTTSTool, project_id=project_id, thread_manager=thread_manager)
    ...
    if "gemini-2.5-flash" in model_name.lower() and "gemini-2.5-pro" not in model_name.lower():
        ...
        default_system_content = get_system_prompt(max_output_tokens=max_tokens, recommended_safe_limit=safe_limit)
    ...
    while continue_execution and iteration_count < max_iterations:
        ...
        # Get max_tokens from the model limits we already calculated
        # max_tokens and safe_limit are already set above when calling get_system_prompt()
