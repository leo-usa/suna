...
ADD COLUMN IF NOT EXISTS prepaid_plan_expires_at TIMESTAMPTZ,
ADD COLUMN IF NOT EXISTS prepaid_payment_method VARCHAR(32);

COMMENT ON COLUMN credit_accounts.prepaid_plan_expires_at IS 'End of prepaid annual plan entitlement (Alipay/WeChat one-time yearly purchase)';
COMMENT ON COLUMN credit_accounts.prepaid_payment_method IS 'alipay or wechat_pay for prepaid annual plans';

CREATE INDEX IF NOT EXISTS idx_credit_accounts_prepaid_expires
ON credit_accounts(prepaid_plan_expires_at)
WHERE stripe_subscription_status = 'prepaid_active';

CREATE OR REPLACE FUNCTION expire_prepaid_annual_plans()
RETURNS INTEGER
SECURITY DEFINER
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    SET
        tier = 'free',
        plan_type = NULL,
        stripe_subscription_status = 'prepaid_expired',
        next_credit_grant = NULL,
        prepaid_plan_expires_at = NULL,
        prepaid_payment_method = NULL,
        updated_at = NOW()
    WHERE stripe_subscription_status = 'prepaid_active'
      AND prepaid_plan_expires_at IS NOT NULL
      AND prepaid_plan_expires_at <= NOW();
...
GRANT EXECUTE ON FUNCTION expire_prepaid_annual_plans() TO service_role;
...
BEGIN
    PERFORM expire_prepaid_annual_plans();
    ...
    FOR v_account IN
        ...
        AND (
            ...
            OR
            (ca.stripe_subscription_status = 'prepaid_active'
             AND ca.prepaid_plan_expires_at IS NOT NULL
             AND ca.prepaid_plan_expires_at > NOW())
        )
        ORDER BY ca.next_credit_grant ASC
    LOOP
        BEGIN
            v_year_end_date := COALESCE(v_account.prepaid_plan_expires_at, v_account.billing_cycle_anchor + INTERVAL '1 year');
            ...
            IF v_months_since_start >= 12 THEN
                RAISE NOTICE '[YEARLY REFILL] Account % has reached 12-month limit (started: %, current: %, months: %), marking for renewal',
                ...
                RAISE NOTICE '[YEARLY REFILL] Set next_credit_grant to year end %. Provider will handle renewal via webhook.', v_year_end_date;
                CONTINUE;
            END IF;

            CASE v_account.tier
                WHEN 'tier_2_20' THEN v_monthly_credits := 40.00;
                WHEN 'tier_6_50' THEN v_monthly_credits := 100.00;
                WHEN 'tier_25_200' THEN v_monthly_credits := 400.00;
            ...
            INSERT INTO public.renewal_processing (
                ...
                v_period_start,
                v_period_end,
                COALESCE(v_account.stripe_subscription_id, v_account.revenuecat_subscription_id, 'prepaid_annual'),
                'cron',
                v_monthly_credits
            );

            RAISE NOTICE '[YEARLY REFILL] Processing account % - tier: %, credits: %, period: % to % (month %/12), provider: %',
                v_account.account_id, v_account.tier, v_monthly_credits, v_account.next_credit_grant, v_next_grant, v_months_since_start + 1,
                COALESCE(v_account.provider, 'stripe');
...
COMMENT ON FUNCTION process_monthly_refills IS 'Processes monthly credit refills for yearly subscribers including prepaid annual (Alipay/WeChat) plans';
