...
async def _latest_assistant_reply(thread_id: str) -> str:
    rows = await threads_repo.get_thread_messages(thread_id, order="desc", optimized=True)
    for msg in rows:
...
async def bridge_chat(
    ...
    reply_text = await _latest_assistant_reply(str(tid))
    if not reply_text:
        reply_text = "（暂无文本回复，请在网页端查看详情。）"

    await _update_link_thread(peer, str(tid))
    if len(reply_text) > 3500:
        reply_text = reply_text[:3490] + "…"
    return BridgeChatResponse(
        ok=True,
        reply=reply_text,
        thread_id=str(tid),
