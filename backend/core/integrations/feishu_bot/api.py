"""
HTTP API for Feishu bot pairing (JWT) and bridge chat (shared secret).

Each basejump account may link one or more Feishu users; pairing ties a
Feishu user id to the workspace selected when generating the code.
"""
...
MSG_BIND_OK = "已成功绑定当前工作区。请继续在飞书中与 Dobby 对话。"
MSG_BIND_INVALID = "验证码无效或已过期。请在本页重新生成验证码，并在飞书中单独发送该 6 位码。"
MSG_BIND_NEEDED = "请先在 Dobby 网页端：用户菜单 → 飞书，选择工作区并生成验证码，然后在飞书中单独发送该 6 位码完成绑定。"
...
def _reraise_if_missing_feishu_tables(exc: Exception) -> None:
    """Clear 503 when migration was not applied to the DB this API uses."""
    msg = str(exc).lower()
    if "does not exist" in msg and "feishu_bot" in msg:
        raise HTTPException(
            status_code=503,
            detail=(
                "Feishu tables are missing on this database. "
                "Apply migration 20260513120000_feishu_bot_integration.sql "
...
class PairingCodeRequest(BaseModel):
    account_id: str = Field(..., description="Workspace (basejump account) to bind this Feishu user to.")
...
class BridgeChatRequest(BaseModel):
    feishu_open_id: str = Field(..., min_length=1, max_length=64)
...
class BridgeChatStartResponse(BaseModel):
    """Immediate response after enqueueing a run (Feishu bridge polls /snapshot + /finalize)."""
...
def _require_bridge_secret(x_feishu_bridge_secret: Optional[str]) -> None:
    expected = (config.FEISHU_BRIDGE_SECRET or "").strip()
    if not expected:
        raise HTTPException(status_code=503, detail="Feishu bridge is not configured")
    got = (x_feishu_bridge_secret or "").strip()
...
async def _clear_link_thread(feishu_open_id: str) -> None:
    sql = """
    UPDATE public.feishu_bot_links
    SET thread_id = NULL, updated_at = timezone('utc', now())
    WHERE feishu_open_id = :feishu_open_id
    RETURNING 1
    """
    await execute_one(sql, {"feishu_open_id": feishu_open_id}, commit=True)


def _feishu_slash_command(message: str) -> Optional[str]:
...
async def _get_link(feishu_open_id: str) -> Optional[Dict[str, Any]]:
    sql = """
    SELECT feishu_open_id, account_id::text AS account_id, thread_id::text AS thread_id
    FROM public.feishu_bot_links
    WHERE feishu_open_id = :feishu_open_id
    """
    return await execute_one_read(sql, {"feishu_open_id": feishu_open_id})


async def _upsert_link(feishu_open_id: str, account_id: str) -> None:
    sql = """
    INSERT INTO public.feishu_bot_links (feishu_open_id, account_id, thread_id, created_at, updated_at)
    VALUES (:feishu_open_id, CAST(:account_id AS uuid), NULL, timezone('utc', now()), timezone('utc', now()))
    ...
    RETURNING 1
    """
    await execute_one(sql, {"feishu_open_id": feishu_open_id, "account_id": account_id}, commit=True)


async def _update_link_thread(feishu_open_id: str, thread_id: str) -> None:
    sql = """
    UPDATE public.feishu_bot_links
    ...
    RETURNING 1
    """
    await execute_one(sql, {"feishu_open_id": feishu_open_id, "thread_id": thread_id}, commit=True)


async def _update_link_thread_resilient(feishu_open_id: str, thread_id: str) -> None:
    """Retry when threads row is not committed yet (FK feishu_bot_links.thread_id → threads)."""
    for attempt in range(24):
        try:
            await _update_link_thread(feishu_open_id, thread_id)
            return
...
async def _consume_pairing_code(code: str) -> Optional[str]:
    sql = """
    DELETE FROM public.feishu_bot_pairing_codes
...
async def _verify_feishu_bridge_run(peer: str, agent_run_id: str, thread_id: str) -> Dict[str, Any]:
    link = await _get_link(peer)
    if not link:
        raise HTTPException(status_code=403, detail="Feishu user is not linked")
    run_row = await agents_repo.get_agent_run_by_id(agent_run_id)
    if not run_row:
...
async def _bridge_start_agent_for_chat(peer: str, raw: str) -> BridgeChatStartResponse:
    ...
    try:
        result = await start_agent_run(
            ...
            thread_id=thread_id,
            project_id=None,
            metadata={"source": "feishu_bot", "feishu_open_id": peer},
        ...
        if isinstance(detail, dict):
            ...
            msg = str(detail)
        logger.warning(f"[feishu_bot] start_agent_run HTTPException: {msg}")
        return BridgeChatStartResponse(ok=False, reply=f"{MSG_RUN_FAILED} ({msg})")
    except Exception:
        logger.exception("[feishu_bot] start_agent_run failed")
...
async def create_pairing_code(
    ...
    try:
        await execute_one(
            "DELETE FROM public.feishu_bot_pairing_codes WHERE account_id = CAST(:aid AS uuid) RETURNING 1",
            {"aid": body.account_id},
            commit=True,
        )
    except Exception as e:
        _reraise_if_missing_feishu_tables(e)
        raise
    ...
    for _ in range(6):
        ...
        try:
            await execute_one(
                """
                INSERT INTO public.feishu_bot_pairing_codes (code, account_id, expires_at)
    ...
    logger.exception("[feishu_bot] failed to generate unique pairing code", exc_info=last_err)
...
async def link_status(
    ...
    try:
        ...
        row = await execute_one(
            """
            SELECT COUNT(*)::int AS c FROM public.feishu_bot_links
...
async def bridge_transcribe(
    feishu_open_id: str = Form(...),
    audio_file: UploadFile = File(...),
    x_feishu_bridge_secret: Optional[str] = Header(None, alias="X-Feishu-Bridge-Secret"),
):
    """Whisper transcription for Feishu voice/audio (bridge secret only)."""
    _require_bridge_secret(x_feishu_bridge_secret)
    peer = feishu_open_id.strip()
    if not peer:
    ...
    try:
        ...
        logger.warning(f"[feishu_bot] bridge_transcribe failed for {peer}: {detail}")
        raise
    except Exception:
        logger.exception("[feishu_bot] bridge_transcribe failed")
...
async def bridge_chat(
    ...
    from the Feishu bridge to avoid proxy timeouts on long tool runs.
    """
    _require_bridge_secret(x_feishu_bridge_secret)
    peer = body.feishu_open_id.strip()
    ...
    try:
        ...
        logger.exception("[feishu_bot] bridge_chat after start_agent_run failed")
...
async def bridge_chat_start(
    ...
    try:
        ...
        logger.exception("[feishu_bot] bridge_chat_start failed")
        return BridgeChatStartResponse(ok=False, reply=MSG_RUN_FAILED)
...
async def bridge_chat_snapshot(
    ...
    try:
        await _verify_feishu_bridge_run(peer, body.agent_run_id.strip(), body.thread_id.strip())
        ...
        logger.exception("[feishu_bot] bridge_chat_snapshot failed")
...
async def bridge_chat_finalize(
    ...
    if not peer or not raw:
        return BridgeChatResponse(ok=False, reply=MSG_BIND_NEEDED)
    try:
        await _verify_feishu_bridge_run(peer, rid, tid)
        ...
        logger.exception("[feishu_bot] bridge_chat_finalize failed")
        return BridgeChatResponse(ok=False, reply=MSG_RUN_FAILED)
